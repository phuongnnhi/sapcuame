...
export const updateOrderStatus = async (orderId: string, status: Order['status']): Promise<Order> => {
    try {
        const response = await apiService.put<Order>(`/order/${orderId}`, { status });
        return response.data;
    } catch (error) {
...
export const getProducts = async (params?: Record<string, any>): Promise<ProductResponse> => {
    try {
        const response = await apiService.get<ProductResponse>('/product', { params });
        return response.data;
    } catch (error) {
...
export const updateProduct = async (productId: string, productData: FormData): Promise<Product> => {
    try {
        const response = await apiService.put<Product>(`/product/${productId}`, productData);
        return response.data;
    } catch (error) {
